...
export class DatabaseStorage implements IStorage {
  ...
  async createStudentProfile(profile: InsertStudentProfile): Promise<StudentProfile> {
    ...
    const [studentProfile] = await db
      ...
      .values({ ...profile, createdAt: now, updatedAt: now } as any)
      .returning();
  ...
  async createJob(job: InsertJob): Promise<Job> {
    ...
    const [newJob] = await db
      .insert(jobs)
      .values({ ...job, createdAt: now, updatedAt: now } as any)
      .returning();
    return newJob;
  }
  ...
  async createApplication(application: InsertApplication): Promise<Application> {
    ...
    const [newApplication] = await db
      .insert(applications)
      .values({ ...application, appliedAt: now, updatedAt: now } as any)
      .returning();
  ...
  async createInterviewSession(session: InsertInterviewSession): Promise<InterviewSession> {
    ...
    const [newSession] = await db
      ...
      .values({ ...session, createdAt: now, updatedAt: now } as any)
      .returning();
    return newSession;
  }
  ...
  async createResumeAnalysis(analysis: InsertResumeAnalysis): Promise<ResumeAnalysis> {
    ...
    const [newAnalysis] = await db
      ...
      .values({ ...analysis, createdAt: now, updatedAt: now } as any)
      .returning();
    return newAnalysis;
  }
  ...
  async createChatMessage(message: InsertChatMessage): Promise<ChatMessage> {
    ...
    const [newMessage] = await db
      .insert(chatMessages)
      .values({ ...message, createdAt: now } as any)
      .returning();
    return newMessage;
  }
